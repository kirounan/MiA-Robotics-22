...
robots_number = int(N_obs/3)
for h in range(robots_number):
    for M in range(N + 1):
        g = ca.vertcat(g, -np.sqrt((X[0:1, M] - obs[3*h]) ** 2 + ((X[1:2, M] - obs[3*h+1]) ** 2)) + (
...
lbg = ca.DM.zeros((n_states * (N + 1) + robots_number*(N + 1), 1))
ubg = ca.DM.zeros((n_states * (N + 1) + robots_number*(N + 1), 1))
...
sim_time = 150  # Maximum simulation time
# Start MPC
mpciter = 0
# xx1 = []
# u_cl = []
...
if __name__ == '__main__':
    ...
    simulate(cat_states, cat_controls, times, h, N,
             np.array([x_init, y_init, theta_init, x_target, y_target, theta_target]), obs, robots_number, save=False)
