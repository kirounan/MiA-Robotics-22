...
for M in range(N + 1):
    g = ca.vertcat(g, -np.sqrt((X[0:1, M] - x_obstcale) ** 2 + ((X[1:2, M] - y_obstcale) ** 2)) + (
                rob_diam / 2) + r_obstacle)
...
if __name__ == '__main__':
    ...
    simulate(cat_states, cat_controls, times, h, N,
             np.array([x_init, y_init, theta_init, x_target, y_target, theta_target]), x_obstcale, y_obstcale,
             r_obstacle, save=False)
